...
node_RB* Insert_node(int elem, node_RB* &parent, node_RB* &ptr) {
    if (ptr == nullptr) {
    ...
    if (ptr->data < elem) {
        return Insert_node(elem, ptr, ptr->right);
    } else {
        return Insert_node(elem, ptr, ptr->left);
    }
}
...
void InsertRB(int elem, node_RB*& parent, node_RB*& ptr) {
    node_RB* new_node = Insert_node(elem, parent, ptr);
    ReDraw(new_node);
    UpdateRoot(ptr);
    return;
}
...
void DeleteRB(int elem, node_RB*& ptr) {
...
int main()
{
    ...
    while (n--) {
        std::cin >> input;
        InsertRB(input, first, first); // first - parent of first - can cause a problem
    }
    
    cin >> n;
    while (n--) {
        std::cin >> input;
        DeleteRB(input, first);
    }
    std::cout << "S";
}